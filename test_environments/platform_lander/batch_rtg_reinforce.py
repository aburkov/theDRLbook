...
trajectory-gradient estimates before each optimizer step. It uses canonical
reward-to-go with the outer ``gamma^t`` factor, matching the discounted-objective
convention used by ``value_function_baseline_reinforce.py``. An entropy bonus can be enabled
with ``--entropy-coef`` to discourage premature policy collapse. The policy
network uses the common OpenAI-Baselines/SB3-style initialization for
on-policy RL: orthogonal hidden layers with gain sqrt(2), zero biases, and a
small 0.01-gain policy-logit head so the initial categorical policy is close
to uniform.

    loss = mean_i [-sum_t gamma^t * G_t^i * log pi(a_t^i | o_t^i)
...
def parse_args() -> argparse.Namespace:
    ...
    add_output_args(parser, "batch_rtg_reinforce")
...
def discount_factors(length: int, gamma: float) -> torch.Tensor:
    """Return [gamma^0, gamma^1, ..., gamma^(length - 1)] as a float tensor."""
    return torch.as_tensor([gamma**t for t in range(length)], dtype=torch.float32)
...
def trajectory_rtg_loss(
    ...
    *,
    ...
    discounts = discount_factors(len(rewards), gamma)
    ...
    policy_loss = -(log_prob_tensor * discounts * rtg).sum()
...
def train(args: argparse.Namespace) -> Policy:
    ...
    try:
        log(
            f"training_start script=batch_rtg_reinforce "
