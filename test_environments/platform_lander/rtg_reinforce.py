...
and no baseline. This version uses canonical reward-to-go with the outer
``gamma^t`` factor, matching the discounted-objective convention used by
``value_function_baseline_reinforce.py``. An entropy bonus can be enabled with
``--entropy-coef`` to discourage premature policy collapse. The policy
...
to uniform.
    ...
    loss = -sum_t gamma^t * G_t * log pi(a_t | o_t)
...
def parse_args() -> argparse.Namespace:
    ...
    add_output_args(parser, "rtg_reinforce")
...
def train(args: argparse.Namespace) -> Policy:
    ...
    try:
        log(
            f"training_start script=rtg_reinforce "
